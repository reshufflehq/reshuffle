...
async function main() {
  ...
  await mssql.query(
  'INSERT INTO Users  values (@name, @age) ',[{name: 'name', type: mssql.Text, value: name}, {name: 'age', type: mssql.Int, value: age}]
  )

  result = await mssql.query(`SELECT * FROM Users where name = @name `, [{name: 'name', type: mssql.Text, value: name}])
  console.log('==> After insert new row: ',result.rows)

  name = 'Tom'
  age = 35
  const params = [{name: 'name', value: name, type: mssql.Text} , {name: 'age', type: mssql.Int, value: age}]
